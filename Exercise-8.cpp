...
float threshold_noise_z = 1;// if the variable z is under the threshold then we consider it as nosie and representing not moving
...
int main(int argc, char* argv[]) {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (_kbhit())// get user's input without blocking  
		{
			currentDirection = Non;
			char cTake = _getche();//get user's input for print out
			std::cout << cTake;
			switch (cTake) {
			case 'd':
